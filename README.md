...
- Home Assistant monitoring via Victron HA integration (Phase 1, no broker required)
- Home Assistant bidirectional control via MQTT (Phase 2)
...
### Primary Target: [LuxOS](https://luxor.tech/firmware)

VenusHashCtrl is designed to integrate with [LuxOS](https://luxor.tech/firmware) first.
...
[Mujina](https://github.com/256foundation/mujina) is an emerging open-source Bitcoin mining firmware from the 256 Foundation targeting multiple ASIC platforms including 256 Foundation's own Libreboard. It represents the longer-term direction for this project:
...
- Waveshare 3-Channel Relay HAT for Raspberry Pi (Direct Bus Mode)
- 200A DC contactor (controlled by relay HAT)
...
- LuxOS — primary firmware target (Ignore PSU mode, per-board control)
- Braiins OS+ — secondary firmware target (power limit API)
- Mujina — future target (open-source, S19-class support pending)
- PyASIC (miner control abstraction layer)
- Home Assistant (Phase 1: monitoring via Victron integration; Phase 2: MQTT control)

---

## Inspiration

Inspired by [@100AcresRanch](https://x.com/100AcresRanch) trailblazing the direct-to-DC mining concepts and the Victron and off-grid open-source communities.
